...
SETTINGS_SCHEMA = {
    ...
    "router.api_key": {"type": "string", "default": "", "secret": True, "group": "core",
                       "description": "Router API key (stored in config, overrides env var)"},
    "router.ninerouter_url": {"type": "string", "default": "", "group": "core",
                              "description": "9Router base URL (NINEROUTER_URL override)"},
    "router.ninerouter_key": {"type": "string", "default": "", "secret": True, "group": "core",
                              "description": "9Router API key (NINEROUTER_KEY override)"},
    "router.openai_key": {"type": "string", "default": "", "secret": True, "group": "core",
                          "description": "OpenAI API key (OPENAI_API_KEY override)"},
    ...
    "skills.environments": {"type": "list", "default": [], "group": "skills",
                            "description": "Active environments for skill matching (v18 I)"},
    "safety.write_approval": {"type": "bool", "default": False, "group": "safety",
                              "description": "Gate memory/skills writes behind two-step approval (Hermes write_approval)"},
    # ---- approvals (Hermes approval.py port, v18 A.11) ----
    "approvals.mode": {"type": "choice", "default": "manual", "choices": ["manual", "smart", "off"],
                       "group": "approvals",
                       "description": "manual = ask on every dangerous action · smart = aux-LLM pre-check · off = skip prompts"},
    "approvals.deny": {"type": "list", "default": [], "group": "approvals",
                       "description": "Glob rules that block commands unconditionally (never bypassable)"},
    "approvals.allowlist": {"type": "list", "default": [], "group": "approvals",
                            "description": "Permanent dangerous-command allowlist (command text or glob)"},
    "approvals.timeout": {"type": "int", "default": 300, "min": 10, "group": "approvals",
                          "description": "Approval prompt timeout seconds (fail closed)"},
    "approvals.cron_mode": {"type": "choice", "default": "deny", "choices": ["deny", "approve"],
                            "group": "approvals",
                            "description": "deny = block dangerous commands in cron · approve = allow"},
    "approvals.smart_policy": {"type": "string", "default": "", "group": "approvals",
